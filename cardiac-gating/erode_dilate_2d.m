function [segm_img, BNmask]= erode_dilate_2d(img_in,SE,thresh);
...
segm_img = abs(img_in); % magnitude image for ease of viewing
    ...
    imshow(segm_img, 'InitialMagnification', 1000, 'DisplayRange', []);
...
[x,y]=meshgrid(-SE:SE,-SE:SE);%, -SE:SE);
nhood=x.^2+y.^2 <=SE^2;%x.^2+y.^2+z.^2 <=SE^2;                % structuring element size
