...
%
% 10/11/2018 Edits figure handle call to operate with current MATLAB
% standard by Zack Cleveland
%
...
% figure_handle = gcf(); % no longer conforms to matlab standard. 
% delete(figure_handle);
% figure(figure_handle);
figure_handle = figure(); %Edited by Zack Cleveland
...
guidata(figure_handle,h);
    ...
    function updateImage()
        ...
        if(isfield(h,'hImage'))
            h.hImage = showImageSlice(h.vol,slice,dim,h.hImage);
        else
            h.hImage = showImageSlice(h.vol,slice,dim);
        end
    end
